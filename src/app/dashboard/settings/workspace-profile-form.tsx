...
import { RiRefreshLine, RiCheckLine, RiLoader4Line } from '@remixicon/react'
...
export function WorkspaceProfileForm({ initialWorkspace }: WorkspaceProfileFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateWorkspaceData(name, slug)
      setSaveSuccess(true)
  ...
  return (
    <Card className="p-6 space-y-6 bg-white border-gray-200 font-satoshi">
        ...
        <div className="flex items-center gap-2">
          {saveSuccess && (
            <span className="text-xs font-semibold text-emerald-600 flex items-center gap-1 font-outfit animate-fade-in">
              <RiCheckLine className="size-4" /> Berhasil Disimpan!
            </span>
          )}
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        {/* Field 1: Nama Workspace (Editable directly) */}
        <div className="space-y-1">
          ...
          <Input
            ...
            className="text-xs text-gray-900 font-medium bg-white border-gray-200 focus:border-indigo-500 focus:ring-indigo-200 font-satoshi"
          />
        </div>
        ...
        {/* Action: Save Button with specific required styling: bg-blue-500 text-black hover:bg-black hover:text-white */}
        <div className="pt-2">
          <Button
            type="submit"
            disabled={isSaving}
            className="bg-blue-500 text-black hover:bg-black hover:text-white transition-colors duration-200 font-semibold text-xs px-4 py-2 rounded-md shadow-sm"
          >
            {isSaving ? (
              <>
                <RiLoader4Line className="size-3.5 animate-spin" /> Menyimpan...
              </>
            ) : (
              'Save'
            )}
